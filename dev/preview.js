...
function PreferenceRegisterExtensionSetting(config) { activePreference = config; config.load(); }
function PreferenceSubscreenExtensionsClear() { activePreference.unload(); activePreference.load(); }
function ServerPlayerExtensionSettingsSync() { log('Settings saved (in memory only)'); }
function ChatRoomRegisterMessageHandler(handler) { handlers.push(handler); }
function ChatRoomSync() {}
function ChatRoomAddCharacterToChatRoom(c) { ChatRoomCharacter.push(c); }
function ChatRoomSyncMemberLeave(data) { ChatRoomCharacter = ChatRoomCharacter.filter(c => c.MemberNumber !== data.SourceMemberNumber); }
function CommonDrawAppearanceBuild() {}
function CharacterRefresh() {}
function InventoryGet() { return null; }
function CharacterSetFacialExpression() {}
function CharacterNickname(c) { return c.Name; }
function CharacterPronounDescription() { return 'They/Them'; }
function ElementValue(id, value) { const n = document.getElementById(id); if (value !== undefined) n.value = value; return n?.value ?? ''; }
function ChatRoomSetTarget(id) { ChatRoomTargetMemberNumber = id; }
...
MainCanvas.canvas.addEventListener('click', e => { const r = MainCanvas.canvas.getBoundingClientRect(); MouseX = (e.clientX - r.left) * 2000 / r.width; MouseY = (e.clientY - r.top) * 1000 / r.height; activePreference?.click(); });
document.getElementById('lang').onchange = e => TranslationLanguage = e.target.value;
document.getElementById('enter').onclick = () => { const c = { MemberNumber: 12345, Name: 'Friend', ArousalSettings: { Progress: 0 } }; ChatRoomSyncMemberLeave({ SourceMemberNumber: c.MemberNumber }); ChatRoomAddCharacterToChatRoom(c); };
function frame() { activePreference?.run(); requestAnimationFrame(frame); } frame();
