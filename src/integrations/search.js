...
export const normalizeSearch = text => [...String(text).normalize('NFKC').toLocaleLowerCase()].map(c => fold.get(c) ?? c).join('').replace(/[\s_:：·'"()（）-]|\[|\]/g, '');
...
export function fuzzyMatch(haystack, needle) {
  const text = normalizeSearch(haystack), query = normalizeSearch(needle);
...
export function createActivitySearch(rows) {
  const indexed = rows.map(row => ({ row, texts: [...new Set([row.label, row.name, ...(row.searchLabels ?? [])].map(normalizeSearch))] }));
...
export function searchActivities(rows, query) {
  ...
  return rows.filter(row => [row.label, row.name, ...(row.searchLabels ?? [])]
    .some(text => fuzzyMatch(text, query)));
}
