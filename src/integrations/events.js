export function installEvents({ sdk, submit, mouth, reset, host = globalThis }) {
  ...
  const event = (kind, actorCharacter, fields = {}) => ({ kind, room: roomKey(), roomName: host.ChatRoomData?.Name ?? '', self: host.Player.MemberNumber, actor: actorCharacter?.MemberNumber, actorCharacter,
    actorArousal: actorCharacter?.ArousalSettings?.Progress ?? 0, selfArousal: host.Player.ArousalSettings?.Progress ?? 0, ...fields });
  const handler = { Description: 'Responsive_Liko: reactions and speech', Priority: 320,
    Callback(data, sender, message, metadata) {
      ...
      } catch (e) { console.warn('Responsive_Liko event', e); }
      return false;
    }
  };
  host.ChatRoomRegisterMessageHandler(handler);
  sdk.hookFunction('ChatRoomSync', 0, (args, next) => {
    syncing++; roomEpoch++; reset();
    try {
      const result = next(args);
      const joined = () => { syncing--; if (host.CurrentScreen === 'ChatRoom') submit(event('event', host.Player, { event: 'join' })); };
      if (result?.then) return result.finally(joined);
      joined(); return result;
    } catch (e) { syncing--; throw e; }
  });
  sdk.hookFunction('ChatRoomAddCharacterToChatRoom', 0, (args, next) => {
    ...
    const result = next(args); const c = host.ChatRoomCharacter.find(c => c.MemberNumber === args[0]?.MemberNumber);
  ...
  sdk.hookFunction('ChatRoomSyncMemberLeave', 0, (args, next) => {
    const c = host.ChatRoomCharacter.find(c => c.MemberNumber === args[0]?.SourceMemberNumber);
    const result = next(args);
    return result;
  });
