...
export function activitySearchLabels(name, group, host = globalThis, character = host.Player, lookup) {
  const english = gameOriginalLabels(host), labels = new Set();
  for (const key of activityLabelKeys(name, group, character)) {
    let translated;
    try { translated = lookup ? lookup(key) : host.ActivityDictionaryText?.(key); } catch { /* resource not ready */ }
    if (!missing(translated)) labels.add(translated);
    if (!lookup) for (const row of host.ActivityDictionary ?? []) if (Array.isArray(row) && row[0] === key && !missing(row[1])) labels.add(row[1]);
...
function* catalogRows(host = globalThis) {
  ...
  const lookup = key => {
    if (!resolved.has(key)) {
      let value;
      try { value = host.ActivityDictionaryText?.(key); } catch { /* resource not ready */ }
  ...
  for (const physical of physicalGroups) {
    ...
    try {
      ...
      for (const item of host.ActivityAllowedForGroup?.(character, physical) ?? []) {
        ...
        found.set(key, { name: activity.Name, group: physical, label: activityLabel(activity.Name, physical, host, character), searchLabels: activitySearchLabels(activity.Name, physical, host, character) });
      }
