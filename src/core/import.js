...
function step(message) {
  if (
    !object(message) ||
    !['message', 'action'].includes(message.type) ||
...
export function importPersonas(text, lz = globalThis.LZString) {
  ...
  if (raw?.format === 'Responsive_Liko' && raw.schemaVersion === 1) {
    ...
    object(raw) &&
    (raw.ResponsesModule || raw.data?.ResponsesModule || Array.isArray(raw.mainResponses))
  ) {
