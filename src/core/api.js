...
export function createAPI(namespace, store, host = globalThis) {
  const listeners = new Set(); const consumers = new Map(); let ready = false; let lastPublished = '';
  let capabilities = { mouth: false, expressions: false }; let desired = { ...capabilities };
  ...
  function publish() {
    ...
    for (const listener of listeners) try { listener(getState()); } catch (e) { console.warn(ID, e); }
    host.dispatchEvent?.(new CustomEvent(`${ID}:state`, { detail: getState() }));
  }
  Object.assign(namespace, {
    apiVersion: 1, version: VERSION, getState,
    isActive: capability => ready && store.data.settings.enabled && capabilities[capability] === true,
    subscribe(listener) { if (typeof listener !== 'function') throw new TypeError('Listener required'); listeners.add(listener); listener(getState()); return () => listeners.delete(listener); },
    ...
    registerConsumer(name, handler) {
      ...
      consumers.set(name, handler); namespace.refresh();
      return () => { consumers.delete(name); namespace.refresh(); };
    },
  });
  return {
    ...
    refresh() {
      ...
      for (const handler of consumers.values()) {
        try { const accepted = handler(clone(desired)); if (accepted !== true) { next.mouth = false; next.expressions = false; } }
        catch { next.mouth = false; next.expressions = false; }
      }
      capabilities = next; publish();
    }
  };
}
