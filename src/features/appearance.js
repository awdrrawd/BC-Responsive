...
export function wearState(host, group, state) {
  if (!state) { host.InventoryRemove?.(host.Player, group, false); return; }
  const worn = host.InventoryWear(host.Player, state.asset, group, state.color ?? 'Default', undefined, undefined, undefined, false);
  const item = worn ?? host.InventoryGet(host.Player, group);
  if (item) { if (state.property !== undefined) item.Property = clone(state.property ?? {}); if (state.craft) item.Craft = clone(state.craft); else delete item.Craft; }
}
...
export async function editAppearanceState(host, group, state, done) {
  ...
  preview.Name = host.Player.Name; preview.AssetFamily = host.Player.AssetFamily;
  ...
  const previewHost = Object.create(host); previewHost.Player = preview;
  const restore = () => { preview.FocusGroup = null; };
  try {
  ...
  } catch (error) { restore(); await host.CommonSetScreen(...screen); await host.PreferenceSubscreenExtensionsOpen?.(ID); throw error; }
}
