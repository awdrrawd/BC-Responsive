...
export function start(namespace, host = globalThis) {
  ...
  const ready = () =>
    host.Player?.MemberNumber !== undefined &&
    host.Player.ExtensionSettings &&
    [
  ...
  function initialize() {
    ...
    try {
      ...
      const valid = (event) =>
        !stopped &&
        store.loaded &&
        store.data.settings.enabled &&
        store.data.settings.reactions &&
        host.CurrentScreen === 'ChatRoom' &&
        host.Player.MemberNumber === account &&
        event.room === events.roomKey() &&
