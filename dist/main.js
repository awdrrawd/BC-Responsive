...
One of mods you are using is using an old version of SDK. It will work for now but please inform author to update`), window.bcModSdk);
  ...
  function lceFeatureEnabled(key, host = globalThis) {
    try {
  ...
  function expressionEngines(host = globalThis) {
    const engines = [];
    try {
      if (host.FBC_VERSION !== void 0 && host.Player?.FBC && host.fbcSettingValue?.("animationEngine") === true && host.fbcSettingValue?.("activityExpressions") === true)
        engines.push("WCE");
    } catch {
    }
  ...
  function expressionConflicts(store, host = globalThis) {
  ...
  // src/core/i18n-engine.js
  var init_i18n_engine = __esm({
    "src/core/i18n-engine.js"() {
      (function() {
        "use strict";
        ...
        installFlags();
        if (window.Liko.__Sys_i18n__ && window.Liko.__Sys_L10N__) return;
        ...
        function installFlags() {
          ...
          function get(country, format) {
            try {
              return cache.get(spec(country, format).key) || null;
            } catch {
              return null;
            }
          }
          ...
          function ensure(country, format = "4:3") {
            ...
            if (failure && Date.now() - failure.time < 5e3) return Promise.reject(failure.error);
            const request = (async () => {
          ...
          const labels = /* @__PURE__ */ new WeakMap();
          const bitmaps = /* @__PURE__ */ new Map();
          function parts(text) {
            const match = String(text).match(/[\u{1F1E6}-\u{1F1FF}]{2}/u);
            if (!match) return null;
            const country = [...match[0]].map((c) => String.fromCharCode(c.codePointAt(0) - 127462 + 97)).join("");
          ...
          function draw(ctx, country, x, y, width, height) {
            ...
            if (!image.pending && Date.now() - image.failedAt > 5e3) bitmaps.delete(key);
            return false;
          }
          ...
          function bindSelect(select) {
            ...
            if (selects.has(select)) {
              selects.get(select)();
              return;
            }
            const original = {
              backgroundImage: select.style.backgroundImage,
              backgroundRepeat: select.style.backgroundRepeat,
              backgroundPosition: select.style.backgroundPosition,
              backgroundSize: select.style.backgroundSize,
              paddingLeft: select.style.paddingLeft
            };
            const savedLabels = /* @__PURE__ */ new Map();
            let sequence = 0;
            let signature = "";
            ...
            const sync = () => {
              ...
              for (const [option2, label2] of savedLabels) {
            ...
            const open = (event) => {
              ...
              Object.assign(menu.style, { left: `${Math.max(4, Math.min(rect.left, innerWidth - Math.max(rect.width, 180) - 4))}px`, top: `${Math.min(rect.bottom + 3, innerHeight * 0.5)}px`, minWidth: `${Math.min(Math.max(rect.width, 180), innerWidth - 8)}px`, maxWidth: "calc(100vw - 8px)", background: style.backgroundColor === "rgba(0, 0, 0, 0)" ? "#222" : style.backgroundColor, color: style.color, font: style.font });
              const controller = new AbortController();
              let removalObserver;
              const close = () => {
                controller.abort();
              ...
              for (const option of select.options) {
                ...
                row.type = "button";
                row.disabled = option.disabled || !!option.parentElement?.disabled;
              ...
              menu.onkeydown = (e) => {
                ...
                if (e.key === "Escape") {
                  ...
                  e.preventDefault();
                  rows[e.key === "Home" ? 0 : e.key === "End" ? rows.length - 1 : (index + (e.key === "ArrowDown" ? 1 : -1) + rows.length) % rows.length].focus();
                }
              };
              document.body.appendChild(menu);
              removalObserver = new MutationObserver(() => {
                if (!select.isConnected || select.style.display === "none") close();
              });
          ...
          const api = {
            ...
            forLanguage,
            supports,
            has: (country, format) => get(country, format) !== null,
            get,
            status,
            ensure,
            create,
        ...
        function normalizeLang(raw) {
          ...
          if (code === "ZH" || low.startsWith("zh")) {
            code = low.includes("tw") || low.includes("hant") ? "TW" : low.includes("cn") || low.includes("hans") ? "CN" : "TW";
        ...
        function detectLang() {
          let raw = "";
          try {
            raw = typeof localStorage !== "undefined" && localStorage.getItem("BondageClubLanguage") || "";
          } catch {
          }
        ...
        const _languageListeners = /* @__PURE__ */ new Set();
        ...
        setInterval(() => {
          ...
          for (const listener of _languageListeners) {
            try {
              ...
              console.warn("[Liko i18n] language listener failed:", e);
            }
          }
        }, 2e3);
        ...
        const _bank = { ui: /* @__PURE__ */ Object.create(null), msg: /* @__PURE__ */ Object.create(null) };
        function _register(realm, ns, strings2) {
          if (!ns || !strings2 || typeof strings2 !== "object") return;
          const store = _bank[realm];
          store[ns] = store[ns] ?? /* @__PURE__ */ Object.create(null);
          for (const [key, langs] of Object.entries(strings2)) {
            if (!langs || typeof langs !== "object") continue;
            store[ns][key] = Object.assign(store[ns][key] ?? /* @__PURE__ */ Object.create(null), langs);
          }
        }
        function _has(realm, ns, key) {
          return !!_bank[realm][ns]?.[key];
        }
        ...
        const _loadedUrls = /* @__PURE__ */ new Set();
        function _bust(url) {
        ...
        function loadScript(url) {
          ...
          _loadedUrls.add(url);
          return fetch(_bust(url)).then((res) => {
            if (!res.ok) throw new Error(`HTTP ${res.status}`);
            return res.text();
          }).then((code) => {
            if (code && !code.trimStart().startsWith("<")) new Function(code)();
            return true;
          }).catch((e) => {
            _loadedUrls.delete(url);
            console.warn(`\u{1F408}\u200D\u2B1B [Liko i18n] \u26A0\uFE0F loadScript ${url}: ${e.message}`);
            return false;
          });
        }
        function _loadJsonLang(realm, ns, lang, url) {
          ...
          return fetch(_bust(url)).then((res) => {
            ...
            _loadedUrls.delete(url);
            console.warn(`\u{1F408}\u200D\u2B1B [Liko i18n] \u26A0\uFE0F json ${url}: ${e.message}`);
            return false;
          });
        }
        ...
        function ui_t(ns, key, vars, forceLang) {
          ...
          if (out == null) {
            console.warn(`\u{1F408}\u200D\u2B1B [Liko i18n] missing key: "${ns}/${key}"`);
            return key;
          }
          return out;
        }
        window.Liko.__Sys_i18n__ = {
          ...
          normalizeLang,
          onChange,
          register: (ns, strings2) => _register("ui", ns, strings2),
          ...
          loadScript,
          // 單一合併 JS
          loadLangs: (ns, urlMap, lang) => _loadLangs("ui", ns, urlMap, lang),
          // 依語言分檔（.js/.json）
        ...
        function msg_t(ns, key, ...args) {
          return msg_tl(detectLang(), ns, key, ...args);
        }
        ...
        function msg_install(modApi) {
          ...
          try {
            ...
            console.warn("\u{1F408}\u200D\u2B1B [Liko L10N] hook \u5931\u6557:", e.message);
          }
        }
        ...
        console.log(`\u{1F408}\u200D\u2B1B [BC i18n] \u2705 v${ENGINE_VER}  loaded (i18n + L10N)`);
      })();
    }
  });
  ...
  var init_fr = __esm({
    "Translation/fr.js"() {
      fr_default = {
        ...
        openActionPicker: "S\xE9lecteur d'actions",
  ...
  function initI18n(host = globalThis) {
    ...
    if (engine?.register && engine?.t) {
      const strings2 = /* @__PURE__ */ Object.create(null);
      for (const [language, table] of Object.entries(tables)) {
        for (const [key, value] of Object.entries(table)) (strings2[key] ??= {})[language] = value;
      }
      engine.register(ID, strings2);
      return (key) => engine.t(ID, key, void 0, normalize(host.TranslationLanguage));
    }
    if (!host.Liko.I18N) {
  ...
  function createPreviewCharacter(host, name) {
  ...
  async function editAppearanceState(host, group, state, done) {
    ...
    const informationReturnScreen = host.InformationSheetReturnScreen ? [...host.InformationSheetReturnScreen] : void 0;
  ...
  function createAnimationPreview(host, now = () => performance.now()) {
    let character, canvas, settings, bounds, started = null, frame = -1;
    ...
    return {
      ...
      play(value) {
        if (!character) return;
        if (!Number.isInteger(value.count) || value.count < 1 || value.count > 100 || !Number.isFinite(value.intervalMs) || value.intervalMs < 1 || value.intervalMs > 12e4 || !value.tracks.length || value.tracks.some((t) => !t.stateA.asset || !animationState(t, "B").asset))
  ...
  var CHARACTER_HEIGHT, CANVAS_UPPER_OVERFLOW, PREVIEW_PADDING;
  var init_animation_preview = __esm({
    "src/ui/animation-preview.js"() {
      init_model();
      init_appearance();
      CHARACTER_HEIGHT = 1e3;
      CANVAS_UPPER_OVERFLOW = 700;
      PREVIEW_PADDING = 20;
    }
  });
  ...
  function installSettings({ store, t, host = globalThis }) {
    ...
    function animationModalBody() {
      return `<div class="rl-animation-layout"><aside class="rl-animation-preview"><canvas width="800" height="1200" data-animation-preview aria-label="${esc(t("preview"))}"></canvas><p class="rl-muted">${esc(t("preview"))}</p></aside><div class="rl-animation-controls"><div class="rl-segments">${animationGroups().map(
        ...
        ""
      )}</div><div class="rl-animation-grid"><span>${esc(t("animationCount"))}</span><input class="rl-input rl-number" type="number" min="1" max="100" data-animation-field="count" value="${modal.count}"><span>${esc(t("animationSeconds"))}</span><input class="rl-input rl-number" type="number" min="0.001" max="120" step="0.001" data-animation-seconds value="${modal.intervalMs / 1e3}"></div><div class="rl-animation-message"><div class="rl-muted">${esc(t("animationMessageHint"))}</div><div class="rl-choice-row">${["chat", "emote", "action"].map((x) => `<button class="${modal.messageType === x ? "on" : ""}" data-animation-message-type="${x}">${esc(t(x))}</button>`).join("")}<div class="rl-choice-tokens"><button data-animation-token="{Self}">${esc(t("insertSelfName"))}</button><button data-animation-token="{Other}">${esc(t("insertOtherName"))}</button></div></div><textarea class="rl-textarea" style="height:90px" data-animation-field="text">${esc(modal.text)}</textarea></div></div></div>`;
    }
    function pickerHtml() {
    ...
    function modalHtml() {
      ...
      if (modal.type === "text")
        body = `<div class="rl-choice-row">${(draft?.trigger.kind === "speech" ? ["chat"] : ["chat", "emote", "action"]).map((x) => `<button class="${modal.responseType === x ? "on" : ""}" data-response-type="${x}">${esc(t(x))}</button>`).join("")}<div class="rl-choice-tokens"><button data-token="{Self}">${esc(t("insertSelfName"))}</button><button data-token="{Other}">${esc(t("insertOtherName"))}</button></div></div><textarea class="rl-textarea" style="margin-top:18px" data-modal-value>${esc(modal.value)}</textarea>`;
