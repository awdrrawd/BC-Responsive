...
test('initial registration uses persisted game language and preserves saved names after language changes', () => {
  ...
  const reopened = createStore(host);
  reopened.load();
  ...
  reopened.load();
  assert.equal(reopened.active.name, '默认');
  ...
  for (const language of ['EN', 'TW', 'CN', 'DE', 'FR', 'RU', 'UA'])
    assert.equal(validateData(defaults(42, language)).personas[0].rules.length, 7);
});
...
test('only visitor events use a rule whitelist; orgasm ignores old member restrictions', () => {
  ...
  const data = defaults();
  const orgasm = data.personas[0].rules.find((r) => r.trigger.kind === 'orgasm');
  orgasm.trigger.members = [99];
  assert.equal(matches(orgasm, { kind: 'orgasm', outcome: 'Orgasmed', actor: 42 }), true);
  assert.equal(
    validateData(data).personas[0].rules.find((r) => r.trigger.kind === 'orgasm').trigger.members,
    undefined,
  );
});
...
test('picker resource work yields before lookup, preserves search results, and cancels closed dialogs', async () => {
  let calls = 0;
  const host = {
    ActivityFemale3DCG: Array.from({ length: 600 }, (_, i) => ({
      Name: `Action${i}`,
      Target: ['ItemHands'],
    })),
    ActivityDictionaryText(key) {
      calls++;
      return key + ' 撥動鈴鐺';
    },
  };
