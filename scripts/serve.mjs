...
function existingPreview(port, rootUrl) {
  return new Promise(resolve => {
    const request = http.get({ hostname: '127.0.0.1', port, path: identityPath, timeout: 1500 }, response => {
      let body = '';
      response.on('data', chunk => { body += chunk; if (body.length > 8192) { resolve(false); request.destroy(); } });
      response.on('end', () => {
        try { const value = JSON.parse(body); resolve(value.app === 'responsive-liko-preview' && value.root === fileURLToPath(rootUrl)); }
        catch { resolve(false); }
      });
      response.on('error', () => resolve(false));
    });
    request.on('timeout', () => { resolve(false); request.destroy(); });
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === identityPath) {
    ...
    res.end(JSON.stringify({ app: 'responsive-liko-preview', root: fileURLToPath(rootUrl) })); return;
  }
...
try {
  ...
  throw new Error(`127.0.0.1:${port} 已被其他服務或舊版預覽占用。請在原服務視窗按 Ctrl+C 停止後，再執行 npm run dev。`);
}
